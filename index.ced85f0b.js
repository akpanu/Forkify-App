...
t9=/(?:ipad|iphone|ipod).*applewebkit/i.test(eu);var t8={};t8="process"===R(O.process);var t6=O.setImmediate,t5=O.clearImmediate,re=O.process,rt=O.Dispatch,rr=O.Function,rn=O.MessageChannel,ri=O.String,ro=0,ra={},rs="onreadystatechange";L(function(){// Deno throws a ReferenceError on `location` access without `--location` flag
...
O.postMessage(ri(e),h.protocol+"//"+h.host)};t6&&t5||(t6=function(e){t4(arguments.length,1);var t=X(e)?e:rr(e),r=t3(arguments,1);return ra[++ro]=function(){tQ(t,void 0,r)},g(ro),ro},t5=function(e){delete ra[e]},t8?g=function(e){re.nextTick(ru(e))}:rt&&rt.now?g=function(e){rt.now(ru(e))}:rn&&!t9?(m=(v=new rn).port2,v.port1.onmessage=rl,g=tZ(m.postMessage,m)):O.addEventListener&&X(O.postMessage)&&!O.importScripts&&h&&"file:"!==h.protocol&&!L(rd)?(g=rd,O.addEventListener("message",rl,!1)):g=rs in ez("script")?function(e){t2.appendChild(ez("script"))[rs]=function(){t2.removeChild(this),rc(e)}}:function(e){setTimeout(ru(e),0)});var rf=(tY={set:t6,clear:t5}).clear;// `clearImmediate` method
...
"object"==typeof globalThis?globalThis.regeneratorRuntime=rb:Function("r","regeneratorRuntime = r")(rb)}// polyfill all async functions to work in older browsers
// this config file stores variables and/or resources that would be used
// app-wide
const r_="https://forkify-api.herokuapp.com/api/v2/recipes/",rw="a666a737-f321-48e4-9f90-f5bbc38022f6",rk=async function(e,t){try{let r=t?fetch(e,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(t)}):fetch(e),n=await Promise.race([r,new Promise(function(e,t){setTimeout(function(){t(Error("Request took too long! Timeout after 10 second"))},1e4)})]),i=await n.json();if(!n.ok)throw Error(`${i.message} (${n.status})`);return i}catch(e){throw e}},rE={recipe:{},search:{query:"",results:[],page:1,resultsPerPage:5},bookmarks:[]},rS=function(e){let{recipe:t}=e.data;// destructuring the data
return{id:t.id,title:t.title,publisher:t.publisher,sourceUrl:t.source_url,image:t.image_url,servings:t.servings,cookingTime:t.cooking_time,ingredients:t.ingredients,...t.key&&{key:t.key}}},rO=async function(e){try{//1. Loading the recipe
// obtain response from the API
let t=await rk(`${r_}${e}?key=${rw}`);rE.recipe=rS(t),rE.bookmarks.some(t=>t.id===e)?rE.recipe.bookmarked=!0:rE.recipe.bookmarked=!1,console.log(rE.recipe)}catch(e){throw console.log(`${e} 💥💥💥💥💥`),e}},r$=async function(e){try{rE.search.query=e;let t=await rk(`${r_}?search=${e}&key=${rw}`);console.log(t),rE.search.results=t.data.recipes.map(e=>({id:e.id,title:e.title,publisher:e.publisher,image:e.image_url,...e.key&&{key:e.key}})),rE.search.page=1}catch(e){throw console.log(`${e} 💥💥💥💥💥`),e}},rj=function(e=rE.search.page){rE.search.page=e;let t=(e-1)*rE.search.resultsPerPage,r=e*rE.search.resultsPerPage;return rE.search.results.slice(t,r)},rL=function(e){rE.recipe.ingredients.forEach(t=>{t.quantity=t.quantity*e/rE.recipe.servings}),rE.recipe.servings=e},rF=function(){localStorage.setItem("bookmarks",JSON.stringify(rE.bookmarks))},rM=function(e){// Add bookmartk
rE.bookmarks.push(e),e.id===rE.recipe.id&&(rE.recipe.bookmarked=!0),rF()},rx=function(e){// Delete Bookmark
let t=rE.bookmarks.findIndex(t=>t.id===e);rE.bookmarks.splice(t,1),e===rE.recipe.id&&(rE.recipe.bookmarked=!1),rF()};!function(){let e=localStorage.getItem("bookmarks");e&&(rE.bookmarks=JSON.parse(e))}();const rP=async function(e){try{// Object.entries converts objects to arrays
let t=Object.entries(e).filter(e=>e[0].startsWith("ingredient")&&""!==e[1]).map(e=>{let t=e[1].split(",").map(e=>e.trim());if(3!==t.length)throw Error("Wrong ingredient format. Pls use the correct format");let[r,n,i]=t;return{quantity:r?+r:null,unit:n,description:i}}),r={title:e.title,source_url:e.sourceUrl,image_url:e.image,publisher:e.publisher,cooking_time:+e.cookingTime,servings:+e.servings,ingredients:t},n=await rk(`${r_}?key=${rw}`,r);rE.recipe=rS(n),rM(rE.recipe);// console.log(state.recipe);
}catch(e){throw e}};var rT={};rT=new URL(E("27Lyk").resolve("eyyUD"),import.meta.url).toString();class rI{_data;// public API
/**
   ...
   */render(e,t=!0){if(!e||Array.isArray(e)&&0===e.length)return this.renderError();this._data=e;let r=this._generateMarkup();if(!t)return r;this._clear(),this._parentElement.insertAdjacentHTML("afterbegin",r)}// public API
/**
   *
   * @param {*} data
   */update(e){this._data=e;let t=this._generateMarkup(),r=document.createRange().createContextualFragment(t),n=Array.from(r.querySelectorAll("*")),i=Array.from(this._parentElement.querySelectorAll("*"));// Updates changed TEXT
n.forEach((e,t)=>{let r=i[t];e.isEqualNode(r)||e.firstChild?.nodeValue.trim()===""||(r.textContent=e.textContent),e.isEqualNode(r)||Array.from(e.attributes).forEach(e=>r.setAttribute(e.name,e.value))});// console.log(curElements);
// console.log(newElements);
}_clear(){this._parentElement.innerHTML=""}renderSpinner(){let e=`
    <div class="spinner">
      <svg>
...
},y=Fraction;class rq extends rI{_parentElement=document.querySelector(".recipe");_errorMessage="We could not find the recipe. Pls try another one";_message="";addHandlerRender(e){// window.addEventListener('hashchange', controlRecipes);
// executing the addeventlistener on multiple events
["load","hashchange"].forEach(t=>window.addEventListener(t,e))}// publisher
addHandlerUpdateServings(e){this._parentElement.addEventListener("click",function(t){let r=t.target.closest(".btn--update-servings");if(!r)return;let{updateTo:n}=r.dataset;// using destructuring
+n>0&&e(+n);// call handler only for 1 or more servings
})}addHandlerAddBookmark(e){this._parentElement.addEventListener("click",function(t){let r=t.target.closest(".btn--bookmark");r&&// console.log(`This is the model`, model.state.recipe);
e()})}_generateMarkup(){return`
...
addHandlerSearch(e){this._parentEl.addEventListener("submit",function(t){t.preventDefault(),e()})}}var rA=new rN,rC=new class extends rI{_parentElement="";_generateMarkup(e){let t=window.location.hash.slice(1);return`
    ...
    `}};class rD extends rI{_parentElement=document.querySelector(".results");_errorMessage="No recipes found for your query. Pls try again, :)";_message="";// _generateMarkup() {
//   return this._data.map(this._generateMarkupPreview).join('');
// }
_generateMarkup(){return this._data.map(e=>rC.render(e,!1)).join("")}}var rR=new rD;class rW extends rI{_parentElement=document.querySelector(".pagination");// publisher
addHandlerClick(e){this._parentElement.addEventListener("click",function(t){let r=t.target.closest(".btn--inline");if(!r)return;// guard condition: if button is not clicked return immediately
console.log(r);let n=+r.dataset.goto;// + converts a number stored as string to a number
console.log("Goto page: ",n),e(n)})}_generateMarkup(){let e=this._data.page,t=Math.ceil(this._data.results.length/this._data.resultsPerPage);// curPage is the current page
return(// if Page 1 and other pages
(console.log("Total number of pages: ",t),1===e&&t>1)?`
    ...
    `:"")}}var rz=new rW;class rU extends rI{_parentElement=document.querySelector(".bookmarks__list");_errorMessage="No bookmarks yet. Find a nice recipe and bookmark it :)";_message="";addHandlerRender(e){window.addEventListener("load",e)}_generateMarkup(){return this._data.map(e=>rC.render(e,!1)).join("")}}var rG=new rU;class rB extends rI{_parentElement=document.querySelector(".upload");_message="Recipe uploaded successfully :)";_window=document.querySelector(".add-recipe-window");_overlay=document.querySelector(".overlay");_btnOpen=document.querySelector(".nav__btn--add-recipe");_btnClose=document.querySelector(".btn--close-modal");// constructor calls addHnadlerShowWindow upon creation of object
constructor(){super(),this._addHandlerShowWindow(),this._addHandlerHideWindow()}toggleWindow(){this._overlay.classList.toggle("hidden"),this._window.classList.toggle("hidden")}_addHandlerShowWindow(){this._btnOpen.addEventListener("click",this.toggleWindow.bind(this))}_addHandlerHideWindow(){this._btnClose.addEventListener("click",this.toggleWindow.bind(this)),this._overlay.addEventListener("click",this.toggleWindow.bind(this))}// publisher
_addHandlerUpload(e){this._parentElement.addEventListener("submit",function(t){t.preventDefault();let r=[...new FormData(this)],n=Object.fromEntries(r);// inner this referring to the form..this obtains the entire form data as an array
e(n)})}_generateMarkup(){}}var rJ=new rB;// import { has } from 'core-js/core/dict';
// console.log(icons);
...
//   module.hot.accept();
// }
const rY=async function(){try{let e=window.location.hash.slice(1);if(!e)return;// guard condition
rH.renderSpinner(),rR.update(rj()),rG.update(rE.bookmarks),//2. Loading the recipe
// obtain response from the API
await rO(e),rH.render(rE.recipe)}catch(e){alert(e),rH.renderError()}},rQ=async function(){try{rR.renderSpinner();//1. get search query
let e=rA.getQuery();if(!e)return;// guard condition
//2. load search results
await r$(e),rR.render(rj()),rz.render(rE.search)}catch(e){console.log(e)}},rV=async function(e){try{rJ.renderSpinner(),await rP(e),console.log(rE.recipe),rH.render(rE.recipe),rJ.renderMessage(),rG.render(rE.bookmarks),// Change the ID in the URL
// this changes the URL in the browswer without reloading the entire page
window.history.pushState(null,"",`#${rE.recipe.id}`),// Close the form window
setTimeout(function(){rJ.toggleWindow()},2500)}catch(e){console.error(`💥💥 ${e}`),rJ.renderError(e.message)}};rG.addHandlerRender(function(){rG.render(rE.bookmarks)}),rH.addHandlerRender(rY),rH.addHandlerUpdateServings(function(e){// update the recipe servings (in the state)
rL(e),rH.update(rE.recipe)}),rH.addHandlerAddBookmark(function(){rE.recipe.bookmarked?rx(rE.recipe.id):rM(rE.recipe),rH.update(rE.recipe),rG.render(rE.bookmarks)}),rA.addHandlerSearch(rQ),rz.addHandlerClick(function(e){rR.render(rj(e)),rz.render(rE.search)}),rJ._addHandlerUpload(rV);//# sourceMappingURL=index.ced85f0b.js.map

//# sourceMappingURL=index.ced85f0b.js.map
